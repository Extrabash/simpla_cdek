...
class CDEK {
    ...
    private $trackUrl = 'http://integration.cdek.ru/status_report_h.php';
    ...
    {
        $this->dateExecute = date('c');
        $data = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>';
        $data .= '<StatusReport Account="'.$this->authLogin.'" Secure="'.$this->_getSecureAuthPassword().'" Date="'.$this->dateExecute.'" ShowHistory="1">';
        $data .= '<Order DispatchNumber="'.$track.'"/>';
        $data .= '</StatusReport>';   
		...
		$result = curl_exec($ch); 
echo '<hr />'.__FILE__.':'.__LINE__.'<pre>'; var_dump($data); echo '</pre>';
var_dump ($ch, $this->trackUrl, $data, $result);
		curl_close($ch); 
